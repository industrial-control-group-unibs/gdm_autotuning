...
if( objArgs( 0 ) == 1 ) //conversione da codice a stringa applicazione
{
	var appType = objArgs( 1 );
	var appFile = objArgs( 2 );
	
	var str = FindAppTypeSTR( appType, appFile );
	if( str == -1 ) WScript.Echo( "Undef" );
...
else
{
	...
	var DevVerTypeRel = objArgs( 6 ); //nome completo del device
}
...
//	Link file simboli plc per SoftScope
var node = gftDocSrc.selectSingleNode( "/devicetemplate/SoftScopeConfig/templatedata/_PREFIX_" );
if( node )
{
	node.setAttribute( "defaultPLCSymTab", "%PCTPATH%\\Service\\SoftScope\\" + DevVerTypeRel + ".syt.xml" );
}

gftDocSrc.save( GFTInpName );

////////////////////////////////////////////////
function FindAppTypeSTR( code, file )
{
	var Mode =
	{
		ForReading   : 1,
	...
	try
	{
		f = fso.OpenTextFile( file, Mode.ForReading );
		if( f == null )
		{
			return -1;
		}
	}
	catch( exc ) //se il file non esiste
	{
		return -1;
	}
	...
	{		
		...
		if( row != "" )
		{   
			var strApp = row.split( "#" )[ 0 ];
			var strNum = row.split( "#" )[ 1 ];
